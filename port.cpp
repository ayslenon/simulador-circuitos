...
void Port_AND::simular(const std::vector<bool3S>& in_port){
    bool3S prov = bool3S::TRUE;
    for(unsigned int i = 0; i < in_port.size(); i++) prov &= in_port[i];
    setOutput(prov);
}
